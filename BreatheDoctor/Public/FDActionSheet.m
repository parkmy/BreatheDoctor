...
- (void)initButtons {
    if (_buttonTitleArray.count > 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, contentViewHeight + .5, contentViewWidth, 50)];
            button.backgroundColor = [UIColor whiteColor];
            button.titleLabel.font = kNSPXFONT(30);
...
- (void)initCancelButton {
    if (_cancelButtonTitle != nil) {
        ...
        _cancelButton.titleLabel.font = kNSPXFONT(30);
        ...
        contentViewHeight += SPACE_SMALL + _cancelButton.frame.size.height + SPACE_SMALL + 5;
    }
}
...
- (void)removeAnimation {
    [UIView animateWithDuration:0.3 delay:0 options: UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(hideSuccess)]) {
            
            [self.delegate hideSuccess];
        }
    }];
}
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(actionSheet:clickedButtonIndex:)]) {
        for (int i = 0; i < _buttonArray.count; i++) {
            if (button == _buttonArray[i]) {
                ...
                _tapActionSheetIndexBlock?_tapActionSheetIndexBlock(i,self):nil;
                break;
            }
        }
    }
...
- (void)cancelButtonPressed:(UIButton *)button {
    if (_delegate && [_delegate respondsToSelector:@selector(actionSheetCancel:)]) {
        ...
        _tapActionSheetIndexBlock?_tapActionSheetIndexBlock(_buttonArray.count,self):nil;
    }
    [self hide];
}
