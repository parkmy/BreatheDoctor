...
- (instancetype)init
{
    return [self initWithDBName:@"UnicomSales"];
}
...
+ (NSString *)getDBPathWithDBName:(NSString *)dbName
{
    ...
    if ([dbName hasSuffix:@".sqlite"] == NO) {
        fileName = [NSString stringWithFormat:@"%@.sqlite", dbName];
    } else {
        fileName = dbName;
    }
//
...
//    NSString *database_path = [documents stringByAppendingPathComponent:dbName];
    ...
    NSString *filePath = [LKDBUtils getPathForDocuments:fileName inDir:DBDIR];
    return filePath;
}
...
- (NSMutableArray *)search:(Class)modelClass tableName:(NSString*)tableName where:(id)where orderBy:(NSString *)orderBy offset:(NSInteger)offset count:(NSInteger)count
{
    return [self searchBase:modelClass tableName:tableName columns:nil where:where orderBy:orderBy offset:offset count:count];
}
...
- (NSMutableArray *)searchWithYourSQL:(NSString *)sql toClass:(Class)modelClass
{
    // replace @t to model table name
...
- (NSMutableArray *)searchWithSQL:(NSString *)sql toClass:(Class)modelClass tableName:(NSString*)tableName
{
    // replace @t to model table name
    //NSString *replaceString = [NSString stringWithFormat:@" %@ ", [modelClass getTableName]];
    ...
    sql = [sql stringByReplacingOccurrencesOfString:@" @t " withString:tableName];
    if([sql componentsSeparatedByString:@" from "].count == 2)
    {
        sql = [sql stringByReplacingOccurrencesOfString:@" from " withString:@",rowid from "];
    }
    ...
    [self executeDB:^(FMDatabase *db) {
        FMResultSet *set = [db executeQuery:sql];
        results = [self executeResult:set Class:modelClass tableName:tableName];
        [set close];
    }];
    return results;
}
...
- (BOOL)insertToDB:(NSObject *)model withTableName:(NSString*)tableName
{
    return [self insertBase:model withTabeleName:tableName];
}
...
- (BOOL)insertBase:(NSObject *)model withTabeleName:(NSString*)tableName
{
    LKDBCheck_modelIsInvalid(model);
    ...
    NSString *db_tableName = tableName;
...
- (BOOL)updateToDB:(NSObject *)model tableName:(NSString *)tableName where:(id)where
{
    return [self updateToDBBase:model tableName:tableName where:where];
}
...
- (BOOL)updateToDBBase:(NSObject *)model tableName:(NSString *)tableName where:(id)where
{
    LKDBCheck_modelIsInvalid(model);
...
{
    return [self deleteToDBBase:model TableName:tableName];
}
...
- (BOOL)deleteToDBBase:(NSObject *)model TableName:(NSString*)tableName;
{
    LKDBCheck_modelIsInvalid(model);
...
//    if([ShareFun isNullOrEmpty:[CDGloablVar sharedGloablVar].DBName])
//    {
//        if([ShareFun isNullOrEmpty:[CDSetting readSessionDoctorID]])return nil;
//        return [[LKDBHelper alloc]initWithDBName:[[CDSetting readSessionDoctorID] stringByAppendingString:@"Comvee"]];
//    }
//    
//    return [[LKDBHelper alloc]initWithDBName:[CDGloablVar sharedGloablVar].DBName];
}
+ (LKDBHelper *)getUsingLKDBHelper:(NSString*)dbName
{
    return [[LKDBHelper alloc]initWithDBName:dbName];
}
@end
