...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        //    mBubbleImageView.image = [[UIImage imageNamed:@"chat_send_nor@2x"]
//        mBubbleImageView.image = [[UIImage imageNamed:@"chat_recive_nor@2x"]
        if (isSender)//是我自己发送的
        {
        ...
        [mImageView autoSetDimension:ALDimensionWidth toSize:120];
...
{
    [mImageView sd_setImageWithURL:kNSURL(model.content) placeholderImage:kImage(@"defaultIconImage")];
...
{
    [self routerEventWithType:EventChatCellImageTapedEvent userInfo:@{kModelKey:self.model,@"self":self}];
}
