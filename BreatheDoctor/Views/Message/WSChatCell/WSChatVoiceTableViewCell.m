...
{
    mSecondLable.text = [NSString stringWithFormat:@"%@'        ",kNSNumDouble(model.voiceMin)];
...
- (void)stopVocieAnimating
{
    if (mVoiceImageView.isAnimating) {
        [mVoiceImageView stopAnimating];
    }
}
- (void)startVocieAnimating
{
...
-(void)voiceBeenTaped:(UITapGestureRecognizer*)tap
{
    ...
    if (mVoiceImageView.isAnimating) {
        [self routerEventWithType:EventChatCellTypeVoicePlayEvent userInfo:@{kModelKey:self.model,@"self":self}];
    }
