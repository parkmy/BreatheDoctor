...
+ (NSMutableArray *)historicalType:(showHistoricalType)type
{
    ...
    if (type == showHistoricalTypePEF) {
        for (int i = 0; i < 3; i++)
        {
            ...
            model.typeTitle = i==0?@"安全":i==1?@"警告":@"危险";
    ...
    }else
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            model.countColor = i==0?[LWHistoricalCountModel normalColor]:i==1?[LWHistoricalCountModel warningColor]:[LWHistoricalCountModel abnormalColor];
            model.typeTag = i;
            [array addObject:model];
        }
    }
...
{
//    NSInteger length = 0;
//    for (LWHistoricalCountModel *model in array) {
//        NSString *count = kNSString(kNSNumInteger(model.count));
