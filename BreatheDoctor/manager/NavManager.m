...
- (void)returnToMainView:(BOOL)isAnimated
{
    ...
    if (self.rootNavigationController.viewControllers.count > 2) {
        [[self rootNavigationController] popToViewController:[self.rootNavigationController.viewControllers objectAtIndex:2] animated:YES];
...
{
    UINavigationController *baseNavCtrl= [[UINavigationController alloc] initWithRootViewController:ctrl];
