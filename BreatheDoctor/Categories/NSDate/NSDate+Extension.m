...
+ (NSString *)timeInfoWithDateString:(NSString *)dateString {
    ...
    if (time < 3600) { // 小于一小时
        ...
        return [NSDate stringWithDate:date format:[self hmFormat]];
    } else if (time < 3600 * 12) { // 小于一天，也就是今天
        retTime = time / 3600;
        retTime = retTime <= 0.0 ? 1.0 : retTime;
        return [NSDate stringWithDate:date format:[self hmFormat]];
    } else if (time < 3600 * 24 * 2) {
        return [NSDate stringWithDate:date format:[self mdFormat]];
    }
    // 第一个条件是同年，且相隔时间在一个月内
    // 第二个条件是隔年，对于隔年，只能是去年12月与今年1月这种情况
        ...
        if (abs(year) <= 1) {
            ...
            if (month == 12 && preMonth == 12) {// 隔年，但同月，就作为满一年来计算
                return [NSDate stringWithDate:date format:[self ymdFormat]];
            }
            return [NSString stringWithFormat:@"%ld-%ld",[date month],[date day]];
        }
